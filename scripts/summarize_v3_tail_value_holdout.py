"""Cross-validate v3 tail/value review candidates by session holdout."""
...
def _candidate_value(
    ...
    *,
    ...
    if candidate is None:
        return baseline
    tail = _float_or_none(row.get(candidate_key))
    return tail if tail is not None else baseline
    ...
    *,
    ...
    for fold in range(fold_count):
        ...
        candidate_by_group = {
            ...
            if row.get("candidate_status") in _APPLIED_STATUSES
        }
        evals = _row_evals(
            holdout_rows,
            group_field=group_field,
    ...
    groups = sorted(
        ...
        key=lambda row: (
            ...
            row["delta_tail_p50_mae"]
            if row["delta_tail_p50_mae"] is not None
            else 0.0,
            str(row["group"]),
        ),
    )
    ...
    print(
        " ".join(
            (
                ...
                f"tail_mae={overall['baseline_tail_p50_mae']}",
                f"candidate_tail_mae={overall['candidate_tail_p50_mae']}",
                f"tail_delta={overall['delta_tail_p50_mae']}",
                f"q6_tail_mae={overall['baseline_q6_tail_p50_mae']}",
                f"candidate_q6_tail_mae={overall['candidate_q6_tail_p50_mae']}",
                f"q6_tail_delta={overall['delta_q6_tail_p50_mae']}",
                f"tail_p90_cover={overall['baseline_tail_p90_coverage']}",
                f"candidate_tail_p90_cover={overall['candidate_tail_p90_coverage']}",
                f"q6_tail_p90_cover={overall['baseline_q6_tail_p90_coverage']}",
                f"candidate_q6_tail_p90_cover={overall['candidate_q6_tail_p90_coverage']}",
            )
        )
    )
    print(
        " ".join(
            (
                ...
                f"tail_mae={candidate_only['baseline_tail_p50_mae']}",
                f"candidate_tail_mae={candidate_only['candidate_tail_p50_mae']}",
                f"tail_delta={candidate_only['delta_tail_p50_mae']}",
                f"q6_tail_mae={candidate_only['baseline_q6_tail_p50_mae']}",
                f"candidate_q6_tail_mae={candidate_only['candidate_q6_tail_p50_mae']}",
        ...
        print(
            " ".join(
                (
                    ...
                    f"tail_below={row['baseline_tail_p50_below_rate']}",
                    f"candidate_tail_below={row['candidate_tail_p50_below_rate']}",
                    f"q6_tail_below={row['baseline_q6_tail_p50_below_rate']}",
                    f"candidate_q6_tail_below={row['candidate_q6_tail_p50_below_rate']}",
                    f"tail_p90_cover={row['baseline_tail_p90_coverage']}",
                    f"candidate_tail_p90_cover={row['candidate_tail_p90_coverage']}",
                    f"q6_tail_p90_cover={row['baseline_q6_tail_p90_coverage']}",
                    f"candidate_q6_tail_p90_cover={row['candidate_q6_tail_p90_coverage']}",
                )
            )
        )
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Cross-validate v3 tail/value review candidates by session holdout.",
    )
    parser.add_argument(
        "paths",
        nargs="*",
