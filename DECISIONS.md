...
**推荐**：把 `200032/200033` 保留为 `random_sample_avg_values` 结构化字段，写入 batch/live 日志，但暂不接入 MC 硬过滤。`200031` 很可能是同系列随机 3 件均价，但当前仍只保留待确认假设。

**用户选择**：先补缺失语义和日志铺垫，再根据更多样本决定是否建立随机样本观测模型。

**取舍**：不会因极端随机样本均价误推整库价值；后续仍能利用它做分布诊断、似然评分或异常风险提示。

**复查点**：继续收集 `200031/200032/200033` 对应 UI 文案；确认随机抽样方式是否有放回、是否按品质或全库均匀抽取。
