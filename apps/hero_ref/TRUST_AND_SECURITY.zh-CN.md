...
BidKing.exe 网络包
  -> WinDivert 本机抓包
  -> scripts/run_windivert_live_monitor.py 解析游戏状态
...
WinDivert 是开源项目，常见 Python 包 `pydivert` 会携带 WinDivert 驱动文件。实际驱动路径可在启动输出里看到，例如：

```text
WinDivert: ...\site-packages\pydivert\windivert_dll\WinDivert64.sys
```

如安全软件拦截，请优先确认 `pydivert` 安装来源和文件路径，再决定是否加入信任。
