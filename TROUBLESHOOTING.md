...
- 后续补充：posterior 现在也输出实验性 `tail_replacement_decision_value` /
  `q6_tail_replacement_decision_value` quantile，但正式出价仍不读取它们；只有存在
  `final_q6_tail_replacement_value > 0` 时才打 replacement miss flag。
...
`aisha_shipwreck_test_sample58_2rounds.json` 最高红为 `永乐大典残本二`，replacement value
为 0；它应归入 `q6_quality_only_deep_local_risk`，不是 replacement miss。
