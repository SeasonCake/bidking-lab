...
- live snapshot / overlay 已把 q6 count/cells 低于 Drop 先验期望的情况转成风险提示：`v2_posterior_rows` 输出 `q6先验缺口`、`q6先验风险参考`、`q6先验风险`，`model_eval.jsonl` 同步结构化字段。悬浮窗只显示黄色风险上界提示，不抬高正式 posterior、bid hint 或 `decision_value`。
