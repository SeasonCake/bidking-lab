...
- 最新非 slow 回归：**548 passed / 13 deselected**；`pyproject.toml` 已配置 `pythonpath = ["src"]`，日常可直接运行 `pytest -q -m "not slow"`，不再需要手动设置 `PYTHONPATH`。
...
- v2 bucket target 已拆分 exact 与 floor：明确 `total_cells/count` 会作为 `total_cells_exact/count_exact` 进入条件采样，`*_min` 与 item-level evidence 仍作为下界。strict exact v1 在 69 份样本下 decision P50 MAE 降至约 28.8 万，但 zero-match 升至 19；下一步要增加“strict exact 零匹配时降级为 floor”的 fallback，并记录哪些约束被放宽。
