...
> **相关文件**：`OBSERVATIONS.md`（技术发现日志）、`TROUBLESHOOTING.md`（踩坑记录）、`docs/project_vision.md`（原始架构设计）、`docs/optimization_roadmap.zh-CN.md`（当前优化路线图）。
...
├── PROGRESS.md           # ← 本文件
├── OBSERVATIONS.md       # 技术发现日志 (46 checkpoints)
└── TROUBLESHOOTING.md    # 踩坑记录 (51条)
```
...
- `pytest -q`：**429 passed**
- `.\scripts\test_smoke.ps1`：**416 passed / 13 deselected**（跳过真实 OCR 图片回归）
...
| Research | ProtoHub / 网络抓包直读 | 先做离线 fixture → `LiveObservationBatch`，只读验证，不做注入/改包/自动竞价 |
...
cd c:\xiangmuyunxing\biancheng\2026\bidking-lab
pip install -e .
.\scripts\test_smoke.ps1  # 日常 smoke，应该 416 passed / 13 deselected
pytest -q                 # 全量，应该 429 passed
