...
- 2026-06-02 live 试用细节补丁：MiniMap 未知品质改为深色斜纹，白色品质改为浅白色块，避免二者混淆；首屏最高价/停止价/风险带改为价格 chips，价值类指标字号加重；detail 层新增 `matplotlib_minimap` 可选渲染，按 contract 的 `min_round=3` 只在 R3+ 点击详情时生成，失败则回退到 Tk Canvas，不影响 mini/hover。
