...
- q6 可规划低估根因继续拆分：posterior/live/batch 新增 q6 件数和格数分位，批评估新增 `q6_plannable_miss_root_causes`。271 份、`--trials 20` 快速扫下，低估不只来自 q6 没采到；`q6_cells_under=92`、`q6_count_under=90`、`low_q6_value_distribution=56`、`mixed_q6_sample_value=44`、`layout_conflict=44`、`below_drop_prior=35`。Aisha shipwreck 仍是第一优先级，说明下一步应处理 q6 件数/格数与 layout footprint 可信度，而不是继续全局抬 q6 出现率。
