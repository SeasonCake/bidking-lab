...
- 最新非 slow 回归：**559 passed / 13 deselected**；`pyproject.toml` 已配置 `pythonpath = ["src"]`，日常可直接运行 `pytest -q -m "not slow"`，不再需要手动设置 `PYTHONPATH`。
...
- 2026-05-31 临时活动口径：`1306003..1306014` 普通十二生肖按 q3 2x2 低价值候选加入 v2 采样，且观测到这些 item anchor 时不再报不可达；`1306001/1306002` 与 `1306015` 仍不加入普通候选。该口径应在活动结束后复查。
