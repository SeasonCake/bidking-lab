...
| 模块 | 状态 | 备注 |
|---|---|---|
| **艾莎/伊森 核心建模** | ✅ ~95% | 截图校准已完成；剩"伊森 R2-R4 条件触发"在 Phase 2 道具组合做 |
| **推断引擎** | ~70% | 紫品 bucket 推断 ✓；剩多 bucket joint posterior（当前 greedy） + 抽检/宝光鉴 per-item observation 接口 |
| **道具 Observation** | ~50% | 均格/估价/扫描已被 QualityBucketObs 字段覆盖；缺"抽检 N 揭示 FULL"和"宝光 N 鉴揭示 quality"两个 per-item 接口 |
| **地图自带信息** | ~30% | `round_category_hints` 入了 schema；今天发现 col[20] 实际是"提示槽位调度器"而非类别名，需 probe 不同主题对应的提示风格。**截图里的具体数值是 session 动态生成**，玩家手动输入 ManualObservation 即可 |
| **可视化 / UI** | ~25% | 2 个 notebook ✓；Streamlit 0% |
| **游戏 patch 兼容** | ~0% | 2026-05-15 patch 把 BidMap 21→23 列+20 张新事件图，parser 还没改；processed maps.json 仍是 patch 前的；project 跑得动但是不能 re-extract |

**整体 ~67%**。

### 用户聚焦：别墅 + 沉船优先（2026-05-15）

- Phase 2 道具组合 contrast MC：只跑别墅 2407 / 沉船 2510 两张（节省 60%+ MC 时间）
- 集装箱（少玩）：简要测试，跑 1-2 张代表图即可
- 快递 / 仓库：略过（入门图，不是简历亮点目标）
- 网吧（极客改造屋）：当作"别墅变种"——其实它是 col[7]=104 (mansion category)，drop_pool 跟别墅同源

### 下一开工的优先序

1. **重写 BidMap parser 兼容 23 列** + 重抽取 maps.json + 重跑 v2 ranking（**必做，半天**）
2. **per-item Observation 接口**：抽检 N（reveals FULL）+ 宝光 N 鉴（reveals quality）→ 接入 inference engine
3. **写 notebook 03_inference_demo**：可视化"无道具" vs "伊森标配" vs "艾莎标配" 三套方案的估价方差对比，**简历首选产出**
4. **Phase 2 contrast MC**：2 英雄 × 3 道具组合 × 2 地图（别墅 + 沉船）= 12 条曲线，输出推荐表
5. **probe col[20] 提示风格**：每个主题的提示规律
6. **joint posterior**（学术优化，可选）
7. **Streamlit UI**（终极简历形态，可选）
...
### `<待 commit>` — docs: detect 2026-05-15 game patch + map-info DSL analysis (2026-05-15)
