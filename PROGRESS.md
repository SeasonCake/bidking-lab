...
- 悬浮窗 UI 已增强：显示 hero/map/round/结算价值、战术摘要、q6 样本率与 q6 价值区间、后验诊断、布局风险和回测误差；q6 低估、layout conflict、exact fallback 等风险会用高亮颜色提示。若 `latest_snapshot.json` 超过 120 秒未更新，会提示检查 Fatbeans 导出或 monitor 进程。`scripts/run_live_overlay.py --demo` 可直接打开内置样本面板，用于无新日志时预览样式。
