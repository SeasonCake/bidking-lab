...
- 悬浮窗 UI 已增强：显示 hero/map/round/结算价值、战术摘要、q6 样本率与 q6 价值区间、后验诊断、布局风险和回测误差；q6 低估、layout conflict、exact fallback 等风险会用高亮颜色提示。若 `latest_snapshot.json` 超过 120 秒未更新，会提示检查 Fatbeans 导出或 monitor 进程。
...
- 2026-05-31 新增样本后，桌面目录 + 项目 copy 共 174 个唯一 Fatbeans JSON，当前代码 `--trials 300` 批评估为 `ok=168, valued=158, zero_match=10, relaxed_exact=11, q6_false_low_risk=12, q6_p90_misses_truth=61, decision_value_mae≈36.9万, value_p90_coverage≈54.4%`。Ethan 别墅已超过 30 份，Ethan 沉船约差 5 份，Aisha 沉船约差 7 份；红货/q6 residual 和 Ethan exact bucket zero-match 仍是主要瓶颈。
...
- `scripts/summarize_live_model_eval.py` 新增 bid_gap 统计，可按 hero / map_family 汇总最高价与结算价值比例、过结算率、停止价与结算差值。当前去重 live log 口径显示 Ethan 局最高价/结算中位比约 1.169，Aisha 约 1.013；沉船约 1.154，别墅约 1.013，支持“Ethan 和沉船对手给价更激进”的观察，但后续应基于重扫后的长期日志继续校准。
