...
- 新增 live monitor / 悬浮 UI first cut：`bidking_lab.live.monitor` 可把 Fatbeans capture 转成 `latest_snapshot.json`、`sessions.jsonl`、`model_eval.jsonl`、`layout_samples.jsonl`；`scripts/run_fatbeans_live_monitor.py` 支持单文件、目录轮询和 stdin payload；`scripts/run_live_overlay.py` 提供一个 tkinter always-on-top 小窗读取最新 snapshot；`scripts/start_live_monitor_overlay.ps1` 可一键启动后台 monitor + 前台悬浮窗。当前数据源仍是 Fatbeans JSON/file/stdin，可替换为未来实时 feed；日志 schema 已包含价值 P50 误差、仓储 P50 误差、布局拟合误差、最高价/抢仓价/停止价与最终价值差。
...
- 采样缺口统计已统一地图族口径：240x/340x/440x 记为 villa，250x/350x/450x 记为 shipwreck，2601 记为 hidden。`collection_readiness` 现在同时给出主要桶 30 局目标和 hidden 每英雄 10 局冷启动目标。源 JSON `--trials 30` 快速扫下，Aisha villa 52、Aisha shipwreck 53、Ethan villa 38、Ethan shipwreck 25、hidden 0；短期优先补 Ethan shipwreck 约 5 份，并为 Aisha/Ethan hidden 各采 10 份。
...
- q6 可规划低估根因继续拆分：批评估新增 `q6_plannable_miss_root_causes`。271 份、`--trials 20` 快速扫下，低估不只来自 q6 没采到；`low_q6_value_distribution=56`、`mixed_q6_sample_value=44`、`layout_conflict=44`、`below_drop_prior=35`。Aisha shipwreck 仍是第一优先级，说明下一步应处理 q6 价值/件数组合与 layout footprint 可信度，而不是继续全局抬 q6 出现率。
- 新增 `scripts/compare_grid_view_reference.py`，只读对照外部 `src/grid_view_v1.3.7` 的 `map_quality_p50_out.csv` 与本项目 Drop/Item/BidMap 推导。2401/2501/4401/4501 的 q5/q6 draw probability、p50 per item、p50 per cell 基本一致，2601 hidden 也只存在很小差异；因此外部项目短期可参考空位 phantom、出价配置和 UI 思路，不需要先反编译 `grid_view.exe`。
