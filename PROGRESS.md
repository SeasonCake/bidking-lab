...
### 推断引擎 backlog（C-31，2026-05-17 审计）

用户拍板前**不动代码**的候选项（P0 优先）：

| 优先级 | 项 | 工作量 | 价值 |
|--------|-----|--------|------|
| **P0** | 秒/放仓：tier 平滑 + 固定种子说明 + 小红仓门控（红 cells≤2 或低总价） | 中 | 恢复 UI 前必做 |
| **P0** | `adaptive_filter` fallback 保留 `huge_cells_override` | 小 | MC 与「★具体巨物」一致 |
| **P1** | UI 文案：均价/均格仅收紧枚举预览，不改变 MC 分位 | 小 | 减少「填了没用」误解 |
| **P1** | 联合约束降级：≥4 项同时填时改 soft score 或提示只保留最强 2–3 项 | 中 | 防 0 候选 / 过锁 |
| **P2** | `huge_cells_override` 进 MC（按 truth 最大单件格数或 huge 物品表） | 大 | 「★游艇」级精确过滤 |
| **P2** | `avg_value` 进 MC | — | **不建议**（见 OBS #31）；保持枚举专用 |
| **P2** | `huge_cells_override` 进 MC | — | **暂缓**；会减少样本、与 band 语义重叠 |
| **P3** | 未知品质形状巨物 → 品质组合先验 | 大 | 与 UI 实验区对接 |
| **P3** | 秒/放仓 cond 链加入紫/金 huge | 中 | 秒/放仓 UI 恢复后再议 |

**已做（C-31）**：秒/放仓 UI 实验下线；OBSERVATIONS / TROUBLESHOOTING #30–31。

**已做（C-31b, 2026-05-17）**：P1 字段作用范围 UI 文案；联合约束 ≥4 项时仅放宽枚举 `avg_value` 容差（MC 不变）。
