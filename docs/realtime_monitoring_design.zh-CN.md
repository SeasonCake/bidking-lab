...
- `external_references/AuctionAnalyzer4.13.3` 是他人的 OCR + 出价计算器，适合参考 OCR ROI、正则解析、均格可达规则和解析枚举；它本身没有 ProtoHub / packet parser。
...
- 已稳定解析：地图、轮次、英雄、出价候选、道具 action、`0x0027` 直接道具响应、公开信息、技能 reveal、全库透视、明镜之眼、结算 inventory。
...
- 进程级 flow 之后还有对局 frame gate：先按应用层长度重组，再只放行
  ...
  当前局内有效 `REV 0x0027` 直接道具响应、`REV push 0x002d`，并要求有效 session id
  或已激活当前 session。商店/账号/设置 JSON、界面交互、
...
- `REV 0x0027` 不重复 session/map/round，parser 会继承当前局上下文。只有 frame 内能解析出
  action result，且 gate 已有当前 session 时才会进入 `tool_revealed`，因此空 ack 和非当前局包不会触发推理。
