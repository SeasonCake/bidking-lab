...
3. 活动 cohort 可以用于 capture/window/truth、鲁棒性测试、source parser/table acquisition 与 shadow-only 调参参考；没有新 drop table 或显式活动映射前，不混入普通沉船校准。
...
6. `v3_prior_*` flat fields 必须由共享 prior helper 生成，archive evaluator 和 live monitor 不得各自手写字段集合。
7. live artifact 与 `model_eval.jsonl` 必须带同一组 `v3_prior_*` / `v3_robust_*` 字段；UI 主建议仍不消费这些字段。

2026-06-07 补充：0605 的 252x 沉船活动 cohort 已统一标记为 `activity_tuning_reference`，scope 为 `source_parser_table_acquisition_and_shadow_tuning_reference_only`，`affects_bid=false`。当前可复核口径是 15 份 capture / 58 个 ready windows；本机 v303 有 `2521-2530` BidMap 但 Drop/source overlay 仍缺，因此它只能作为调参参考和表源审计，不作为 default prior、readiness/promotion 或正式出价证据。

### 2026-06-06 prior-stress audit

新增 `summarize_v3_prior_robustness_audit.py` 后，prior robustness 不再只是 readiness 总门槛，而可以按 status、reason、map、hero/profile 分片审计。

当前 64-trial archive 观察：
...
1. prior-stressed 行不能直接进入普通 sampler calibration 分母。
2. `summary_likelihood + total/q6 cells above prior` 更像 evidence/prior mismatch，需要先判断是有效 hard evidence、旧表漂移、活动机制，还是 parser 映射异常。
3. `q6_value_above_prior` 行 below/P90 更差，是 formal/value sampler 的高风险审计分片，但仍不能直接上修。
4. 活动 cohort 必须保持 `post_ready=0`、`metric_rows=0`、`fallback=missing_prior_truth_only`，只用于鲁棒性和活动映射，不计普通模型准确率。
5. 后续 formal/value sampler 的训练/holdout 分母至少要并列报告 trusted-only、weak-fallback、prior-stressed 三个口径。
