...
| `tests/test_summarize_v3_promotion_readiness.py` | v3 formal promotion readiness 总审计测试，覆盖 prior-stress detail contract、v3 practical archive-live guard brief contract checks、v2/guarded/unguarded policy matrix、CSE shadow artifact contract、SCP guarded bridge stability artifact contract 与 shadow sampler prototype/guard-trial/value-source-profile/value-map-profile-details/value-profile-guardability contract |
| `tests/test_summarize_v3_promotion_workbench.py` | v3 lane-level promotion workbench 测试，覆盖 stop-loss lane、archive quality watch、shadow sampler contract 边界与 support-aware prototype/guard-trial/value-source-profile/value-map-profile-details/value-profile-guardability blocker |
...
| `scripts/summarize_live_windivert_brief.py` | live/archive brief | 输出 practical candidate/raise-watch rate、practical MAE/delta/under-rate、practical P90 coverage/extreme-over，以及 raise-watch hit/miss/false-alarm/extreme-over/misleading rate；archive replay 支持 `--archive-formal-mode v2|v3_practical`，默认保持 builder v2 对照口径；summary/group stats 输出 formal-mode counts、v3 practical live guard rows/rate、paired guarded-vs-unguarded MAE/P50/P90/P90-coverage/P90-extreme-over deltas，以及 v2/guarded/unguarded 同分母 `formal_policy_comparison`；v303 BidMap parser fallback 与 activity missing-drop alias 后，72h archive `v3_practical` replay 可跑通 |
