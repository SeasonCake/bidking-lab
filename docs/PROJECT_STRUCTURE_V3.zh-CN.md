...
| `scripts/summarize_v3_shadow_sampler_value_source_profile_audit.py` | 从 guarded trial JSON 汇总 q6_value 残余 blocker 的 map/profile/movement/source-profile 迁移，比较 baseline 与 audit-probe artifact，并判断是否需要 q6_value source/profile parser 或更高层 value guard |
...
| `tests/test_summarize_v3_promotion_workbench.py` | v3 lane-level promotion workbench 测试，覆盖 stop-loss lane、archive quality watch、shadow sampler contract 边界与 support-aware prototype/guard-trial/value-source-profile blocker |
...
| `tests/test_summarize_v3_shadow_sampler_value_source_profile_audit.py` | v3 CCVC q6_value source/profile residual audit 测试，覆盖复合 label 解析、baseline/probe 风险迁移与 source-profile parser required 判定 |
