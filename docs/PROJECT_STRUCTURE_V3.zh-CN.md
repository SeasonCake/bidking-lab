...
| `scripts/summarize_v3_shadow_sampler_prototype.py` | v3 `v3_ccvc_` evidence-driven count/cell/value shadow sampler prototype 聚合审计，合并 row contract、shadow safety、policy/direction holdout、evidence contribution、component-level blocker、per-seed watch label/support metrics 与 multi-seed stability |
...
| `tests/test_summarize_v3_shadow_sampler_prototype.py` | v3 CCVC shadow sampler prototype 聚合审计测试，覆盖 watch candidate、shadow safety、component status、per-seed watch label/support metrics 与 seed stability |
