...
| `scripts/summarize_live_windivert_brief.py` | live/archive brief | 输出 practical candidate/raise-watch rate、practical MAE/delta/under-rate、practical P90 coverage/extreme-over，以及 raise-watch hit/miss/false-alarm/extreme-over/misleading rate；archive replay 支持 `--archive-formal-mode v2|v3_practical`，默认保持 builder v2 对照口径；summary/group stats 输出 formal-mode counts、v3 practical live guard rows/rate、paired guarded-vs-unguarded MAE/P50/P90/P90-coverage/P90-extreme-over deltas；v303 BidMap parser fallback 与 activity missing-drop alias 后，72h archive `v3_practical` replay 可跑通 |
