...
| 路径 | 作用 | 当前策略 |
| --- | --- | --- |
| `scripts/run_live_overlay.py` | 当前 UI overlay | UI 设计冻结，不做视觉重做；hover/detail/alert 显示 shadow-only `v3_practical_*` 实战参考，compact 主决策仍用 baseline/fallback；v3 实战参考区显示 `ΔP50/ΔP90/Δq6/Δq6P90`，便于 P90-only ceiling 在实战中可见 |
| `scripts/run_windivert_live_monitor.py` | WinDivert live monitor | 保持当前路径；v3 shadow artifact/model_eval 输出 `v3_robust_*`、`v3_capacity_*`/cases、`v3_fv_*`、`v3_scp_*`、含 source context/pressure tier 的 `v3_cse_*` 与 diagnostics-only `v3_practical_*`；canonical evidence events 会传入 practical 层用于 random avg 等 source-aware reference；random avg high-signal P90 ceiling、low-support q6 raw-tail ceiling 与 residual q6 value ceiling 只进入 practical shadow |
