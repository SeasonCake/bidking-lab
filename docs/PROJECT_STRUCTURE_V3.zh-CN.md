...
| `scripts/summarize_v3_settlement_count_prior_candidates.py` | v3 settlement occupancy count prior shadow-only 候选审计，按 map/prefix/family/residual-mode/round/session/capture-day/session-token-prefix 维度统计 final inventory count、临时生肖扣除 residual、runtime/item duplicate、unique non-temp item cap coverage、reachable Drop item-universe 覆盖、0x002D outer wrapper、payload field-shape、occupied/empty slot shape、candidate path、slot headroom、public-total/full-action evidence 与 current BidMap/round-cap 覆盖 |
