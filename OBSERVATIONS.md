...
- 2026-06-01 q6 可规划口径拆分：
  - batch evaluator 现在同时记录 raw `final_q6_value` 和裁尾后的 `final_q6_decision_value`；
    `final_q6_trimmed_tail_value` 用于把未被证据确认的百万级红货视为 ceiling/risk，而不是常规出价主线。
  - 全量 271 份唯一 Fatbeans JSON、`--trials 40` 快速扫：`q6_truth_files=227`、
    `q6_p90_misses_truth=115`、`q6_value_p90_coverage≈49.3%`；可规划口径下
    `q6_plannable_truth_files=224`、`q6_plannable_p90_misses_truth=108`、
    `q6_plannable_value_p90_coverage≈51.8%`。
  - q6 尾部事件 13 份，裁掉的 q6 尾部中位值约 `123.7万`。这说明黑天鹅尾部确实会污染 raw
    coverage，但裁尾后只改善约 2.5 个百分点；Aisha shipwreck 仍是可规划 q6 residual 的第一优先级。
  - `q6_plannable_calibration_priority` 当前排序：Aisha shipwreck miss rate 约 `65.8%`，
    Aisha villa 约 `41.8%`，Ethan shipwreck 约 `35.9%`，Ethan villa 约 `35.0%`。
