...
- `capture/screen.py`：`INFO_PANEL_CROP_FRAC = (0.17, 0.07, 0.52, 0.72)`（相对**主屏**宽高）。
- `list_monitors()` / `capture_monitor_png_bytes()`：依赖可选 `mss`；默认主显示器；副屏用同一比例相对该屏分辨率裁剪。
- 参考分辨率 `1920×1080` 用于文档与暖机；其它分辨率仍按分数裁剪。
