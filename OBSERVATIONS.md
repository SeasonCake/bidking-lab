...
| [`OBSERVATIONS.md`](OBSERVATIONS.md) | 本文件；记录实验结果、指标、根因分析和 checkpoint 技术细节。 |
| [`TROUBLESHOOTING.md`](TROUBLESHOOTING.md) | 记录可复用的故障排查手册，只收关键 bug / 环境坑 / 回归模式。 |

使用关系：本文件保留“为什么这么判断”的证据；若证据变成正式取舍，写入 `DECISIONS.md`；若证据代表当前路线或完成度变化，摘要写入 `PROGRESS.md`；若形成可复用修法，整理到 `TROUBLESHOOTING.md`。
...
- 外部参考项目 `external_references/grid_view_v1.3.7/` 只读初查：
...
- 2026-06-01 q6 可规划低估根因继续拆分：
  ...
  - posterior/live/batch 现在输出 q6 件数和格数分位：`q6_count`、`q6_cells`、
    `v2_q6_count_p90_under_by`、`v2_q6_cells_p90_under_by`。这能区分 q6 低估是件数、格数、
    还是同格数下候选价值偏低。
  - `QualityDropPrior` 同步输出 `expected_session_count`、`expected_session_cells`；
    live/batch 记录为 `v2_q6_prior_expected_count`、`v2_q6_prior_expected_cells`。这用于对照
    “Drop 先验本来就低”与“evidence/layout 把 q6 count/cells 压低”。
  - 271 份、`--trials 20` 快速扫中，可规划 q6 低估根因前列为：
    `q6_cells_under=92`、`q6_count_under=90`、`q6_count_below_prior=83`、
    `q6_cells_below_prior=70`、`low_q6_value_distribution=56`、`mixed_q6_sample_value=44`、
    `layout_conflict=44`、`below_drop_prior=35`。这说明下一步不能只提高 q6 出现率；还要处理
    q6 count/cells、layout footprint 可信度和剩余空间/件数组合。
  ...
  - 新增 `q6_count_cell_prior_floor_experiment` 离线 what-if：只在 q6 count/cells P90 低于 Drop
    先验期望时，用 q6 先验价值做 floor。`floor_ratio=0.5` 仅把可规划 q6 coverage 从约 `46.6%`
    提到 `49.8%`；`floor_ratio=1.0` 可提到约 `58.9%`，但会覆盖 `26` 个无可规划 q6 的 eligible
    行。该路径仍不应直接进正式 posterior，只适合作为后续 Aisha shipwreck 风险提示或更细门控实验。
  - 新增 `q6_count_cell_prior_gated_floor_experiment`，按 hero+map_family 只保留改善数大于
    无 q6 误触发代理数的门控组合。`floor_ratio=1.0` 下选中 `Aisha+shipwreck`
    与 `Ethan+shipwreck`：可规划 q6 coverage 约 `57.1%`，miss 从 `117` 降到 `94`，
    eligible 无 q6 代理为 `8`。对比全局 floor 的 coverage `58.9%` / eligible 无 q6 `26`，
    窄门控更适合作为实战风险候选；villa 组暂不进入门控。
  - live monitor 已把 q6 count/cells P90 低于 Drop 先验期望的情况输出成结构化风险字段：
    `q6先验缺口`、`q6先验风险参考`、`q6先验风险`；shipwreck 正净收益门控局额外输出
    `q6实战门控`、`q6实战参考P90`，并写入 `model_eval.jsonl` 的
    `q6_count_cell_prior_*` 字段。overlay 会显示黄色“q6 件数/格数低于先验”提示；
    `summarize_live_model_eval.py` 同步统计风险次数、参考上界中位数、实战门控次数和分组风险率。
    这只是风险上界，不改变后验分位、`decision_value` 或 bid hint。
  - 评估器和 live 日志现在按轮次与信息密度分层：
    `evidence_stage=early_1_2/mid_3_4/full_5`；
    `information_density_score/band` 综合轮次、anchor、shape/category target、反排与可信 footprint。
    271 份、`--trials 20` 快速扫：`early_1_2=49`、`mid_3_4=192`、`full_5=23`；
    信息密度为 `low=40`、`medium=125`、`high=99`。高信息局当前 `decision_value_mae≈43.9万`，
    medium 约 `37.1万`，low 约 `33.8万`，说明高信息样本本身更偏复杂/高价值/高约束；
    不能直接解释成“信息越多越差”。评估器已新增 `density_value_tier` 和 q6 按信息密度分组，
    后续应按地图族、价值档、信息密度联合看准确率。
  - q6 实战门控现在可以用 live 日志评估真阳性/假阳性：
    `q6_practical_gate_under_before` 表示门控前 q6 P90 低于结算 q6，
    `q6_practical_gate_covered_after` 表示实战参考 P90 能覆盖结算 q6，
    `q6_practical_gate_helped` 表示该门控确实把一次低估补住，
    `q6_practical_gate_false_positive_proxy` 表示门控命中但结算无 q6。
    `summarize_live_model_eval.py` 会按 hero、hero+map_family、轮次和信息密度汇总这些字段。
    现有 `data/logs/live/model_eval.jsonl` 有效 140 行，但都是旧字段，门控统计为 0；
    需要后续新版 monitor 实测局来观察真阳性/假阳性。
  - 新增 `evidence_profile_key`，用于按证据类型拆分准确率：`public:max_quality`、
    `public:max_item_cells`、`public:random_avg`、`tool:category`、`shape`、`layout`。
    这覆盖了用户提到的最高品质、最高占格、随机均价、鉴影分类、形状/local 等主要输入。
  - 同时修正了批评估中 public bonus 的计算顺序：之前 `public_constraint_key` 在信息密度分数之后才写入，
    导致 public 证据没有真正提高 density score。
  - 271 份、`--trials 20` 快速扫：信息密度现在为 `low=38`、`medium=124`、`high=102`。
    按英雄看，`Aisha|high` 有 93 份、decision MAE 约 `44.6万`，`Ethan|medium` 71 份、
    decision MAE 约 `32.0万`。这说明英雄和信息密度必须联合分析；艾莎高信息局通常也是更难的局。
  - q6 按证据 profile 看，`shape+layout`、`public:random_avg+shape+layout`、
    `tool:category+shape+layout` 仍有明显可规划 q6 miss。结论不是“这些证据没用”，而是当前 sampler
    对“已知形状/位置/均价后剩余 q6 件数与格数”的处理还不够强。
  - `q6_count_cell_prior_profile_gated_floor_experiment` 按 `hero+map_family+evidence_profile`
    进一步细化 floor 门控，并要求每个 profile 组合至少 10 个可规划 q6 truth。当前保留的稳定组合为：
    `Aisha+shipwreck+shape+layout`、`Ethan+shipwreck+layout`、
    `Aisha+shipwreck+tool:category+shape+layout`、`Ethan+shipwreck+shape+layout`。
    该门控把 eligible 无 q6 代理从 hero+map 门控的 `8` 降到 `6`，但 coverage 也从约 `57.1%`
    降到约 `54.8%`。因此它暂时作为诊断候选，不替换 live shipwreck 门控。
  - 新增 `q6_actionable_targets`，自动把 q6 分层摘要转成“下一步建议”。当前前列为：
    `evidence_profile=shape+layout` -> `remaining_space_feasibility`；
    `Aisha+shipwreck` -> `shipwreck_q6_count_cell_gate`；
    `information_density=high` -> `high_density_layout_conflict_audit`；
    `Aisha+shipwreck+shape+layout` -> `shipwreck_shape_space_residual`。
    这说明后续主线应转向空间/剩余 q6 件数组合，以及高信息局 layout conflict 可信度分层。
  - q6 plannable 分组摘要已补空间压力字段。`shape+layout` 组：`q6_plannable_truth=111`、
    miss `56`、`q6_count_under=46`、`q6_cells_under=49`、`q6_count_below_prior=43`、
    `q6_cells_below_prior=38`、layout conflict `68`、可信 footprint 中位 `27`、已占格中位 `70`。
    `public:random_avg+shape+layout` 和 `tool:category+shape+layout` 也有类似 count/cell under。
    这验证了“看见很多形状/位置后仍低估 q6”的主因更像剩余空间与 q6 件数组合，而不是缺解析字段。
  - v2 posterior 已新增剩余空间轻量诊断：对每个匹配样本计算
    `truth.warehouse_total_cells - known_layout.occupied_cells`，并输出
    `remaining_cells_after_layout`、`q6_space_pressure` 和 `q6_space_overflow_rate`。
    其中 q6 空间压力使用扣除已知 q6 anchor 后的 residual q6 cells，避免把已经占位的红货
    再拿去挤占剩余空间。
    live snapshot 与 `model_eval.jsonl` 同步记录这些字段；batch evaluator 的 q6 plannable
    分组摘要也输出剩余空间中位数、q6 空间压力 P90 中位数和溢出率均值。
    这为后续格子装箱可视化和空间约束评分提供数据契约，但当前不作为硬过滤。
  - 新增 `q6_space_diagnostics` 后，`--trials 20` 快速扫显示：219 个可规划 q6 truth 中
    miss 117 个；miss 的 q6 空间压力 P90 中位 `0.171`，covered 的中位 `0.294`；
    低空间压力 miss 90 个，高空间压力/溢出 miss 14 个。Aisha shipwreck + shape+layout
    是最典型分组：38 个 miss 里 35 个低空间压力，0 个高空间压力，压力 P90 中位 `0.081`。
    结论：当前 q6 低估主因不像“剩余空间装不下”，更像 residual sampler 没采足 q6 件数/格数/价值组合。
  - `q6_low_space_residual_floor_experiment` 验证了另一个边界：低空间压力本身不是足够安全的门控。
    `floor_ratio=1.0` 下，低空间 floor eligible truth 116、eligible 无 q6 代理 26、coverage `0.5799`；
    加 hero+map+profile 正净收益门控后，eligible truth 66、无 q6 代理 6、coverage `0.5434`。
    对照现有 `q6_count_cell_prior_profile_gated_floor_experiment` 为 eligible truth 74、无 q6 代理 6、
    coverage `0.5479`。结论是：空间压力字段用于解释和筛查，不直接替代 profile gate。
  - `q6_residual_boost` 实验把上述 floor 诊断转成 sampler 层可控开关。默认 `1.0` 不改变生产路径；
    boost 只作用在 residual sampling 的 q6 候选权重，不影响 anchor、shape target、category target。
    `--trials 20` 快速扫结果：
    baseline q6 plannable coverage `0.4658`、miss `117`、decision MAE `39.2万`、
    无可规划 q6 但 q6 P90 正报率 `0.7742`、正报中位 `18.5万`；
    全局 boost=3 coverage `0.6532`、miss `77`、decision MAE `35.7万`，
    但无 q6 正报率升到 `0.9355`、正报中位 `37.5万`；
    `shipwreck_profile_v1` gated boost=3 coverage `0.5837`、miss `92`、decision MAE `37.2万`，
    无 q6 正报率仍 `0.7742`、正报中位 `25.4万`；
    gated boost=5 coverage `0.6018`、miss `88`、decision MAE `36.6万`，无 q6 正报指标未继续恶化。
    这说明 profile gate 能保留大部分收益并控制误报代理，但仍需更高 trials 和 live 日志确认。
  - summary 已新增 `q6_residual_boost_experiment`，用于追踪每次 boost 实验的激活范围。
    `shipwreck_profile_v1 + b=5` 下 active rows 为 113/264，active no-q6 rows 为 8；
    激活 profile 为 Aisha shipwreck shape+layout、Aisha shipwreck tool:category+shape+layout、
    Ethan shipwreck layout、Ethan shipwreck shape+layout。该块后续可直接作为 sampler 实验对比入口。
  - 新增 `scripts/compare_q6_residual_boost.py` 后，boost 稳定性可以复现为一条命令。
    该脚本同一批样本依次跑 baseline、global b=3、profile b=3、profile b=5，
    并输出相对 baseline 的 coverage/miss/MAE/no-q6 proxy delta。2 份样本、`--trials 5`
    冒烟通过；正式评估建议用 `--trials 80` 或更高。
  - 复查 `--trials 80` 全量命令时发现其默认工作量为 271 份唯一样本 × 4 套配置 × 80 trials，
    即约 86,720 个 posterior trial；命令不是死锁，但旧版只在结束时输出 JSON，8 分钟内没有反馈。
    脚本已补 stderr 进度、工作量提示与 `--configs` 筛选；后续先用
    `--configs baseline profile_b5` 做稳定性基线，再决定是否补完整四配置对比。
  - live monitor 现在会把 `profile_b5` 作为 q6 residual boost shadow 写入 artifact 和
    `model_eval.jsonl`，但不改变 baseline posterior、`decision_value` 或 `bid_rows`。
    shadow 统计包含激活状态、evidence profile、q6 决策 P90、q6 count/cells P90、
    相对 baseline 的 q6 P90 delta、under-before / covered-after / helped 与无 q6
    false-positive proxy。新版采样目标按用户最新口径改为 Aisha/Ethan 沉船各 20 份、
    Aisha hidden 10 份、Ethan hidden 5 份。
  - q6 boost 对比脚本已补 paired 指标，不再只看汇总 coverage。271 份、`--trials 20
    --configs baseline profile_b5` 下，`profile_b5` paired 修复 30 个 q6 低估，未新增
    q6 newly-missed，也未新增 no-q6 positive；但仍有 87 个 q6 miss 未修复。另测
    public-normalized shipwreck profile 候选会扩大激活到 public:* + shape/layout 局，
    但整体 coverage 和 MAE 不如当前 v1 gate，暂不采纳。
  - 2026-06-01 晚间同步桌面源目录后，项目 copy 与源目录均为 311 份 JSON。当前
    `--trials 20 --configs baseline profile_b5` 下，baseline 的可规划 q6 coverage 为
    `0.4585`、miss `137`、decision MAE `38.93万`；`profile_b5` 为 coverage `0.6260`、
    miss `95`、decision MAE `35.59万`。paired 审计修复 42 个 baseline q6 低估，
    没有新增 q6 newly-missed 或 no-q6 new-positive；无 q6 正报率不变，但正报金额中位
    从 `18.0万` 升到约 `30.86万`，所以仍只保留 shadow。
  - 用户指出低值随机均价、Aisha 底部藏货、Ethan 少带鉴影和单轮样本偏差后，批评估链路已补
    对应诊断。随机均价 `< 20000` 仍保留 raw 日志，但不再参与 q6 profile 路由；
    `200036` 作为紫品均价 soft target 接入。全量 Aisha 沉船中，`bottom_row >= 16`
    风险组 q6 miss 为 `21/23=91.3%`，低于阈值组为 `50/78=64.1%`；这只能作为风险提示，
    不能直接写成抬价规则。当前还有 early rows `56`、single-round rows `19`、
    early large cells gap rows `19`，校准 MAE 应继续和早轮诊断拆开看。
  - q6 收数报告已拆口径：batch 的 `q6_shadow_reference_coverage` 统计全部离线 JSON；
    live 的 `q6_shadow_sampling_progress` 只统计带 `profile_b5` shadow 字段的新日志，
    旧 schema 日志不会计入。当前项目没有新版 `model_eval.jsonl`，所以 live shadow 实战
    收数仍应从 0 开始；离线参考样本覆盖为 Aisha shipwreck `112`、Ethan shipwreck `70`、
    Aisha hidden `0`、Ethan hidden `5`。
  - 当前 public-info 语义审计已覆盖 17 个 id：`pending_model_ids=[]`、`unknown_ids=[]`、
    `needs_screenshot_ids=[]`。换言之，现有样本里的公共信息都有唯一对应的模型语义或
    诊断语义；后续只有遇到新 id、字段冲突或样本 truth 对不上时才需要追加截图验证。
  - `200048` 和 `200050` 从“事后 global evidence 降权/拒绝”前移到 sampler 候选预筛：
    `200048` 排除超过最高品质的候选，`200050` 排除大于最大占格的候选，等于最大格仍允许。
    这修正了“200050 只能排除更大格子的红，不排除小红”的口径：当前正是按最大格上界筛桶，
    不把小红从 residual bucket 里删掉。
  - 公共均格 fixed32 已拆成 total/all 与品质桶两种诊断：`200014` 作为整局平均格数，
    `200013/200015/200016` 分别作为 q4/q5/q6 平均格数。`200038` 也按 q6 均价/均值口径
    进入 soft target。模型现在利用这些 float 给 posterior 加权，但不默认硬过滤。
  - 均格唯一性审计显示：精确 float 能消除“两位小数 OCR 多解”的一层噪声，但数学上仍常有
    倍数解。当前 311 份文件、304 份 v2 ok 样本里，avg-cells 诊断为 `unique=3`、`ambiguous=48`；
    其中 total/all avg cells 为 `unique=3 / ambiguous=9`，q4 为 `ambiguous=19`，
    q5 为 `ambiguous=20`。示例：`2.596491` 可唯一到 `57 件 / 148 格`，
    但 `2.204082` 同时允许 `49/108` 与 `98/216`。
  - 为人工审查新增 compact sample export：`scripts/export_fatbeans_review_samples.py`
    把长 JSON 裁成 `data/review/fatbeans_compact/summary.csv`、逐局 `events/*.jsonl`
    和 `errors.jsonl`。本轮导出 306 份成功、5 份 malformed frame-length 报错，便于后续按
    public info、均格、q6 miss 或极端局快速抽样复核。
  - `case_tags` 已进入批评估行级输出，并在 summary 中拆出 normal/single-round/tail/
    no-q6-control/zero-q6-proven/high-info-miss/avg-cells unique/ambiguous。311 份、
    `--trials 5 --configs baseline profile_b5` 下，baseline normal-case q6 可规划覆盖
    `0.2819`、miss `135`；`profile_b5` 为覆盖 `0.4439`、miss `104`，paired 修复
    normal q6 低估 `31` 行，但新增 normal no-q6 positive `3` 行。high-info q6 miss
    从 `79` 降到 `66`。结论：boost 方向有效，但仍不能进入正式出价。
  - baseline high-info q6 miss 根因非常集中：79 行中 Aisha shipwreck `56`、Aisha villa
    `20`、Ethan villa `3`；证据 profile 以 `shape+layout` 为主（54 行），其次
    `tool:category+shape+layout`（14 行）。根因 top 为 q6 count under `66`、cells under
    `64`、count below prior `66`、cells below prior `56`；public max quality/item cells
    各只有 3 行，avg-cells unique 只有 1 行。这支持“v2 证据层暂不推翻，重点修 residual
    q6 count/cells sampler”的判断。
  - 新增默认关闭的 sampler 级 `q6_residual_prior_floor_ratio` 实验，让 gated profile 先采到
    q6 Drop 先验比例的件数/格数。`aisha_shipwreck_profile_v1`、ratio `1.0` 下，整体
    q6 可规划覆盖从 baseline `0.3534` 提到 `0.5600`，high-info q6 miss 从 `79` 降到 `39`，
    normal-case MAE 从 `42.66万` 降到 `35.35万`；但 no-q6 control 正报率升到 `0.7097`，
    正报中位约 `35.0万`。ratio `0.5` 仍有类似 no-q6 正报副作用。因此该实验只能作为诊断，
    不进入正式 posterior、live 或 bid hint。
  - no-q6 误抬审计显示，`profile_b5` 和宽 prior-floor 的 Aisha 误抬样本多集中在
    `bottom_row=7..12`，而不是底部风险局。阈值扫描后新增 `aisha_shipwreck_deep_v1`
    gate：Aisha + shipwreck + shape/layout profile + `bottom_row >= 13`。311 份、`--trials 80`
    三配置复测下，`profile_b5` 的 q6 coverage 最高（`0.6792`，paired 修复 43 行），
    但 no-q6 正报金额中位从 `18.72万` 升到 `25.23万`；`aisha_deep_floor1`
    coverage `0.6298`、paired 修复 30 行、无 newly-missed、无 no-q6 new-positive，
    且 no-q6 中位不变，decision MAE `38.10万 -> 33.04万`，normal q6 覆盖
    `0.4350 -> 0.5750`，high-info q6 miss `67 -> 45`。因此当前更适合把
    `aisha_deep_floor1` 作为可升级候选，`profile_b5` 保持更激进 shadow。
  - live monitor 已把 `aisha_deep_floor1` 接成第二套 shadow 字段：artifact 保留
    `q6_residual_boost_shadow` 兼容旧 `profile_b5`，新增 `q6_residual_deep_floor_shadow`
    和 `q6_residual_sampler_shadows`；`model_eval.jsonl` 新增 `q6_residual_deep_floor_shadow_*`
    平铺字段，汇总脚本会单独统计 active/helped/false-positive 和 Aisha shipwreck 收数进度。
    该 shadow 只用于实战验证，不改变 baseline posterior 或 bid hint。性能审计显示两套 shadow
    均激活且 `n_trials=500` 时单样本未限流约 `26.6s`，因此 live shadow 默认 cap 为
    `min(n_trials, 80)`；限流后同样本 smoke 约 `10.0s`，需要更高稳定性时再用
    `--shadow-trials` 显式提高。
  - 进一步性能拆分显示，当前 live monitor 是“文件落盘后同步生成完整 artifact”，不是每帧
    UI 实时推断。两套 shadow 都激活的 Aisha shipwreck 样本上：`n_trials=20` 约 `1.05s`，
    `n_trials=80` 约 `1.87s`，`n_trials=500 + shadow cap 80` 约 `10.0s`，
    `n_trials=500 + shadow_trials=1` 约 `7.4s`，强制 `shadow_trials=500` 约 `23.7s`。
    因此当前瓶颈主要是主路径 `500 trials` 的同步后验/仓储估计，shadow cap 已把额外成本压到
    约 2-3 秒。若未来要做 UI 即时交互，应优先把正式 live 参数降到 80-200 trials 或做后台异步，
    而不是继续压 shadow。
  - `aisha_deep_floor1` 剩余 miss 继续审计后，没有找到低风险放宽 gate。`--trials 20`
    下 111 个 q6 miss 里，当前 active gate 只剩 11 个；非 active 的 100 个主要分散在
    Aisha shipwreck `shape+layout`（21）、Ethan shipwreck `layout`（18）、
    Aisha villa `shape+layout`（16）和 Ethan shipwreck `shape+layout`（6）。把 prior-floor
    放宽到 `shipwreck_profile_v1` 可多修 37 个、放宽到 `aisha_shipwreck_profile_v1`
    可多修 22 个，但 no-q6 正报金额中位都会升到约 `37.68万`，active no-q6 行为
    `10/10` 或 `8/8` 正报。因此当前不继续扩大 sampler gate。
  - live 汇总新增 `q6_shadow_candidate_readiness`，把候选升级状态收敛成四类：
    `needs_live_samples`、`blocked_false_positive`、`no_observed_help`、
    `candidate_for_review`。其中 `aisha_deep_floor1` 独立使用 Aisha shipwreck 20 份目标；
    达到目标且没有 false-positive、同时观察到 helped 后，才进入人工升级复核。正式 UI 的
    “baseline 先显示、shadow 后台补齐”适合作为后续交互性能方案，当前只记录规划。
  - live artifact / `model_eval.jsonl` 已补性能字段：`processing_seconds`、
    `monitor_n_trials`、`monitor_shadow_trials`、`monitor_roi_trials`。汇总脚本输出
    `monitor_processing_seconds_median/p75` 和 trials 分布。真实 Aisha shipwreck smoke
    在 `n_trials=80` 下记录 `processing_seconds≈1.87s`，summarizer 能正确汇总为
    `monitor_processing_seconds_median≈1.872`。
  - 新增 hidden 样本后，项目样本数增至 329，compact review 导出为 324 份成功、5 份
    malformed 原始抓包错误。hidden 已从 `normal_case` 中剥离为 `hidden_case`，避免
    `2601 隐秘拍卖会` 的高红/高价值分布污染普通局指标。
  - hidden 审计显示 Aisha 与 Ethan 差异很大：22 份 hidden 中 21 份有 q6；`trials=80`
    下 Aisha hidden q6 coverage 只有 `10%`、decision MAE 约 `150.7万`，Ethan hidden
    q6 coverage 为 `100%`、decision MAE 约 `22.1万`。Aisha hidden miss 主要是
    `shape+layout` profile 下 q6 count/cells 低估，并伴随大量 layout conflict。
  - 新增默认关闭的 `aisha_hidden_floor1` 候选：Aisha + hidden + `shape+layout` 时启用
    q6 prior-floor ratio `1.0`。Aisha hidden 定向 `trials=80` 下 MAE `150.7万 -> 66.95万`、
    q6 coverage `10% -> 60%`。全 329 样本 `trials=20` paired 对照中，它只激活
    11 个 Aisha hidden 行，active no-q6 为 0，normal-case 指标不动，hidden MAE
    `91.3万 -> 51.4万`，hidden q6 coverage `52.38% -> 66.67%`。
  - live monitor 已新增 `q6_residual_hidden_floor_shadow_*`，与 `profile_b5`、
    `aisha_deep_floor1` 并列写入 `q6_residual_sampler_shadows`。真实 smoke 中
    Aisha hidden 会激活该 shadow，Aisha shipwreck sample71 不会误激活 hidden shadow。
  - 固定对比入口新增 `aisha_deep_hidden_floor1` 离线组合候选。全 329 样本、`trials=40`
    下，baseline decision MAE 为 `41.999万`、q6 coverage `47.83%`；组合候选为
    decision MAE `34.425万`、q6 coverage `60.14%`。paired 审计显示修复 34 个 q6 低估，
    没有 q6 newly-missed 或 no-q6 new-positive；normal MAE `40.992万 -> 35.044万`，
    hidden MAE `86.745万 -> 45.927万`，active rows `58`、active no-q6 rows `0`。
    这支持把组合候选作为未来正式升级评估口径，但当前 live 仍保留三套独立 shadow 以便分开收证据。
  - Aisha hidden still-missed 审计显示，`aisha_hidden_floor1` 触发范围正确但强度偏弱：
    replay 下 active rows `11`、under-before `9`、helped `2`、still-missed `7`、
    false-positive `0`。ratio 扫描显示 Aisha hidden `trials=80` 下 ratio `1.25`
    q6 coverage `80%`，ratio `1.5` 与 `2.0` 都为 `100%`；全 329 样本 `trials=20`
    下 ratio `1.5` 只激活 11 个 Aisha hidden 行，normal MAE/q6 coverage 不变，
    active no-q6 rows 仍为 `0`。因此 live hidden shadow 改为 `aisha_hidden_floor15`，
    但仍不进入正式 posterior 或 bid hint。
  - `aisha_hidden_floor15` live replay 复核 22 份 hidden artifact（`n_trials=80`、
    `shadow_trials=80`）后，sampling progress 已达 Aisha hidden 目标；candidate readiness
    为 `candidate_for_review`，active rows `11`、under-before `9`、helped `7`、
    still-missed `2`、false-positive proxy `0`，`helped_rate=0.7778`。两份剩余 miss
    分别是 `aisha_hidden_test_sample5_5rounds.json` 和
    `aisha_hidden_test_sample7_4rounds.json`，shadow 已把 q6 P90 提高到约
    `279.4万` / `227.4万`，但仍低于最终 q6 `307.7万` / `295.3万`；问题更像隐藏图高价值尾部
    仍偏保守，而不是 public-info 映射、hero/map/轮次 truth 或 gate 误触发。
  - 2026-06-02 新增 9 份 Isabella / Wuqilin Villa 样本后，发现旧 Fatbeans hero parser
    只识别 Aisha/Ethan，会把 Isabella/Wuqilin 局解析成 `hero=null` 或错误套用对手 Aisha/Ethan。
    修正后 9 份样本均能正确进入 v2/live artifact。
  - Isabella 的 `100110` 技能块语义等同“本场最高品质藏品”：若该 skill item quality 为 5，
    即可证明 q6=0。两份 Isabella no-q6 样本中，`public_max_quality:5` 后 q6 P90 均为 `0`；
    其中 sample6 同时有 `public_max_item_cells:8`。该结论支持把 skill-based max-quality
    与 `200048` 共用全局上界路径。
  - Wuqilin no-q6 Villa 样本只有 `public_max_item_cells:6`，没有最高品质上界，因此仍不能排除
    1格/2格/4格小红；baseline q6 P90 约 `15.9万` 属于预期保守误抬。它是 normal no-q6
    校准样本，不是 hidden floor 的反例。
  - 两份单格小红样本验证了另一侧边界：真实 q6 count/cells 为 `1/1` 时，当前 baseline q6 P90
    会偏高（约 `64.4万`、`74.0万`），但 `profile_b5`、`aisha_deep_floor1`、
    `aisha_hidden_floor15` 三套 shadow 均未激活，没有把 Villa 小红局进一步误抬。
  - 主线回归加入 9 份新 Villa 对照后，338 样本 `trials=20` 下窄门控 floor 候选仍保持
    active no-q6 `0`：`aisha_deep_floor1` 修复 28 个 q6 低估，`aisha_hidden_floor15`
    修复 9 个；新增 Villa no-q6 / 单格小红没有进入这两个 gate。宽 `profile_b5` 仍会把
    no-q6 正报金额中位从约 `18.5万` 抬到 `30.7万`，所以只保留激进 shadow。
  - Aisha shipwreck 定向 `trials=80` 对照（111 份输入、110 份可解析）中，
    `aisha_deep_floor1` 把 q6 coverage 从 `38%` 提到 `68%`，miss `62 -> 32`，
    decision MAE `47.45万 -> 33.80万`；active rows `47`、active no-q6 `0`，
    paired helped `30`、newly-missed `0`、no-q6 new-positive `0`。
  - `aisha_deep_floor1` 剩余 11 个 historical replay still-missed 呈混合根因：部分 q6
    count/cells P90 仍偏低；部分件数/格数已不低，但真实 q6 单价属于极端高尾。继续统一提高
    floor 会混淆“缺件数”和“高单价尾部”，不满足低风险升级条件。
  - watch 模式现在会把处理失败的 Fatbeans JSON 写入 `monitor_errors.jsonl`，并在
    `processed_files.json` 中用当前 size/mtime 指纹标记 `status=error`。这解决了
    malformed 抓包在实时目录里被无限重试的问题；同名文件内容变化、`--reprocess` 或显式
    `--retry-errors` 仍可用于调试和重跑。汇总脚本会默认读取同目录
    `monitor_errors.jsonl`，输出错误行数、唯一坏文件指纹数、错误类型分布和最近错误。
  - CLI 级 watch smoke 使用 1 个有效 Villa 样本和 1 个 malformed JSON 验证通过：
    有效样本正常写 `model_eval.jsonl`，坏样本写 `monitor_errors.jsonl` 且同指纹二次轮询跳过。
    `q6_residual_boost_shadow_trials` 已补入 `model_eval.jsonl`，与 deep/hidden shadow 的
    trials 字段一致。
  - `ui_contract` 已作为 `latest_snapshot.json` 的正式 UI 接入边界：baseline 区域标记
    `official=true / affects_bid=true`，q6 风险参考和全部 shadow 均标记 `affects_bid=false`。
    overlay 现在会读取该契约展示 shadow 风险参考；真实样本 smoke 确认 `profile_b5`、
    `aisha_deep_floor1`、`aisha_hidden_floor15` 都能稳定出现在契约里。
  - minimap 已进入同一 UI 契约：`minimap_grid_items` 从最新 grid batch 导出全部已知物品，
    `ui_contract.minimap` 汇总 known items、quality/category counts 和每个 footprint。
    真实 Villa 样本 smoke 产出 `known_items=35`；overlay model 能生成 MiniMap 区块并保留
    baseline 首屏决策。hover/detail 交互先在 `ui_contract.interaction` 中预留字段边界。
  - minimap 视口已改成稳定工程版：默认 `10` 列、`130` 格，契约保留 `max_cells=250` 和
    `scrollable` 标记。matplotlib 可用，但暂不放入首屏同步渲染；`interaction.detail.renderers`
    只标记 `matplotlib_minimap / optional_async / min_round=3`，后续在详情层异步接图。
  - `ui_contract` 继续补齐推理输入展示层：`truth` 只代表结算/样本回放可知信息；
    `constraints.summary` 汇总输入总格/总件数、已知品质件数、shape/category/exclusion 计数；
    `baseline.posterior` 暴露 q6 件数/格数、剩余空间、空间压力和 total-cells 预测。这样 UI
    可以把首屏、hover、点击详情分层，而不需要直接解析完整 artifact/raw rows。
  - live monitor 对 session totals 的处理已改为“只恢复可信 pre-settlement 约束”：全量轮廓或
    透视在结算前给出的 `warehouse_total_cells` / `total_item_count` 会进入
    `inference_input_constraints` 并作为 v2 baseline 输入；结算 inventory 状态会被标为
    `settled`，即使 message_id 不是 `0x002D` 也不会进入 pre-settlement。真实 smoke 中
    `aisha_villa_test_sample60_5rounds.json` 无公开总件数，仍为 `session_totals_stripped`；
    package12/package17 分别恢复 `42/123`、`50/157`。
  - zero-match 根因审计确认三类 confirmed defects：runtime evidence 跨轮次合并保留旧
    `local_index` 会制造假 layout overlap；全局总件数+总格数只近似填充会被可见轮廓下界
    再次过滤；残余抽样会继续加入已满足 exact bucket/value target 的品质，破坏桶约束。
    修复后 338 样本 UI readiness batch（`n_trials=20 / shadow_trials=20 / roi=0`）
    `zero_posterior_match_rows=0`、`fallback_active_rows=0`，`layout_conflict_rows`
    从 129 降到 13。fallback 仍保留为兜底，但当前样本已不需要触发。
  - Ethan 切片当前为 131 行：`zero_posterior_match=0`、`fallback_active=0`、
    `layout_conflict=0`，剩余 `q6_below_drop_prior=8`。按 review 的
    `decision_value` P50 对结算值粗算，Ethan MAE 约 `37.70万`，q6 value MAE
    约 `32.65万`。这说明本轮已修复 Ethan 的“无解/假布局冲突”主 bug，但 Ethan
    高约束样本的 q6 残差和风险提示仍需继续校准，不能宣称推理完全解决。
  - layout conflict 第二轮审计发现，13 行剩余冲突全部来自 evidence-store 全事件合并，
    而不是 latest pre-settlement minimap 真实重叠。Ethan 6 行 latest batch 均无 overlap；
    冲突样本中，shape-bearing 证据的 `local_index=None` 应按 Fatbeans protobuf 默认值
    解释为左上角 `0`，但 shape-less 质量/估价 action 的 local 被 merge 到同 runtime
    形状证据上，导致大件被错误移动并覆盖小件。修复后 readiness batch
    `layout_conflict_rows=0`、`zero_posterior_match_rows=0`、`fallback_active_rows=0`。
  - q6 below-prior 分层显示该 flag 需要降噪：新增 review-only
    `q6_below_drop_prior_class/actionable/under_by` 字段后，可直接把粗 flag 拆成
    真漏、0 红噪声和已覆盖三类。进一步修正 v2 诊断：当 `public_max_quality < 6`
    或 Isabella 最高品质技能证明无红时，不再生成 q6 below-prior。当前 54 行 below-prior
    中 41 行是真实 q6 P90 漏真值，8 行 truth q6 为 0，5 行 truth q6 已被 P90 覆盖。真实漏估主要集中在
    Aisha shipwreck 24、Aisha villa 11、Ethan villa 5、Ethan shipwreck 1；
    中位 under-by 约 `39.81万`，最大为 `134.98万`。因此后续应把
    `q6_below_drop_prior` 从“采样率低于 Drop 先验”拆成“真实/高风险漏估候选”
    与“真实 0 红或已覆盖的诊断噪声”。
  - 338 样本、`trials=20` 的 paired shadow 对比：baseline `zero_match=0`、
    decision MAE `38.38万`、q6 coverage `55.25%`；`aisha_deep_floor1` 保持
    `zero_match=0`，paired 修复 27 个 q6 漏估、newly-missed 0、no-q6 new-positive 0，
    decision MAE 降到 `33.74万`，normal-case MAE `37.67万 -> 31.92万`。
    `aisha_hidden_floor15` 把 hidden q6 coverage `59.09% -> 100%`、hidden MAE
    `79.36万 -> 37.54万`，但该离线 compare 有 1 个 shadow zero-match，因此仍只作为
    hidden shadow，不进入正式 baseline。
  - q6 actionable review 新增 shadow 覆盖状态后，41 个真实 q6 P90 漏估拆为
    active shadow candidate `18`、not covered by current shadow gate `23`。Aisha
    shipwreck 是当前可被 `aisha_deep_floor1` 解释的主要组：24 个 miss 中 18 个 active，
    6 个不在 gate 内；Aisha villa 11、Ethan villa 5、Ethan shipwreck 1 全部未被当前
    gate 覆盖。该结果支持“不扩大现有 floor gate，先把 unsupported miss 单独审计”的路线。
  - 接口审计发现 `model_eval.evidence_profile_key` 曾从 UI 摘要行重算，漏掉 layout 维度，
    而 q6 shadow gate 使用的是 problem 级 profile，导致 review 分组与实际 gate 不一致。
    修复后 profile 单一来源改为 live artifact 的 `evidence_profile_key_from_problem(problem)`；
    readiness batch 仍为 333 行成功、5 个 malformed 错误，q6 actionable profile 分布修正为
    `shape+layout=27`、`tool:category+shape+layout=6`、`layout=5`、
    `public:random_avg+shape+layout=2`、`public:max_item_cells+shape+layout=1`。
  - full review 慢的主要原因不是 Fatbeans 解析或 v1 map/warehouse，而是重复 v2 posterior：
    60 样本 profile 中 baseline v2 约 `33.9%`、boost shadow `26.4%`、floor shadow
    `24.5%`；解析约 `1%`，v1 map/warehouse 合计约 `8.9%`。重复构建
    `ResidualProblem` 约 `10%`，可作为后续优化，但第一优先级是减少不必要 shadow。
    `profile_b5` 是 debug-only，review 默认跳过后全量导出从约 `147.5s` 降至
    `115.3s`，q6 actionable / layout / zero-match summary 不变。
  - not-covered q6 miss 的首轮 ratio 实验显示：Aisha Villa `shape+layout` 的 prior-floor
    `0.5/0.75` 有收益且当前同 profile no-q6 new-positive 为 0；但 not-covered 子集仍有
    4 行高尾 miss，说明它只能作为 shadow 候选，不应直接 baseline 化。Ethan `layout`
    floor 能显著减少 miss，但 `ethan_villa_test_sample41_4rounds.json` 会从 q6 P90 0 抬到
    约 `68.7万`，属于明确误抬风险，暂不设 gate。
  - Aisha Villa `shape+layout` 已收窄成 `aisha_villa_floor05` pending shadow。全量 compare
    中 ratio `0.5` 与 `0.75` 都修复 7 个 q6 漏估且没有 no-q6 new-positive；但 `0.75`
    对尾部和已有 no-q6 正报金额的抬升更激进，因此 live 只接 `0.5`。review 复跑后
    `q6_actionable_shadow_status` 从 23 个 not-covered 拆出 9 个
    `active_pending_shadow_candidate`，全部来自 Aisha Villa `shape+layout`；剩余未覆盖集中在
    Aisha shipwreck 非 gate profile、Ethan layout / Villa 组和少量 public avg/max-cells profile。
    这支持继续把 q6 风险优化做成分组 shadow，而不是把 `q6_below_drop_prior` 直接变成统一抬价。
  - Aisha shipwreck low-bottom 行不能简单并入 deep floor。剩余 6 个 not-covered Aisha
    shipwreck miss 的 `layout_bottom_row` 为 `9-12`；宽 `aisha_shipwreck_profile_v1`
    floor05 在全量 paired 中修 28 行，但 active no-q6 为 8，且 no-q6 正报中位从约
    `21.5万` 抬到 `38.1万`。改成两阶段 “baseline `q6_below_drop_prior` + Aisha
    shipwreck profile” 后，floor05 只激活 21 行、修 12 行，仍有 1 个 active no-q6
    边界样本。该结果说明 bottom-row 不是唯一问题：低底行 shipwreck 既有真实漏红，也有无红局；
    需要额外证据才能区分，当前不应新增 low-bottom shadow。
  - UI review CSV 现在导出 q6 gate 审计字段：`layout_bottom_row`、
    `q6_prior_expected_count/cells/value`、`q6_count_p90_under_prior_by`、
    `q6_cells_p90_under_prior_by`、`q6_count_cell_prior_gap/floor_value`。
    这些字段用于调参和人工复核，不改变 `ui_contract` 首屏字段或正式 baseline。
  - q6 actionable followup bucket 已把剩余风险变成可行动队列：当前 41 个真漏中，
    `shadow_observation=27`、`aisha_shipwreck_low_bottom_floor_risky=6`、
    `aisha_villa_public_profile_outside_pending_gate=2`、`ethan_layout_floor_risky=5`、
    `ethan_non_layout_floor_risky=1`。这意味着当前 q6 主线的剩余工作不应再按总数推进，
    而应按桶推进：先补 public-profile Villa 的对照，再处理 Ethan layout 的误抬边界，
    shipwreck low-bottom 在没有额外证据前只做风险解释。
  - 2026-06-02 追加桶级复测：Aisha Villa public profile 只在 9 个同 profile 样本激活，
    ratio `0.5/0.75/1.0` 最多修 2 个 q6 漏估，但会把两个 max-cells no-q6/control
    样本抬到约 `43-61万` 以上，因此不能并入当前 `aisha_villa_floor05`。Ethan layout
    在 53 个同 profile 样本激活，ratio `0.5/0.75/1.0` 修 7-9 行，但 5 个
    active no-q6/no-plannable control 被抬高；其中 4 个是真 q6=0，另 1 个是 q6 真实存在但
    全部属于超高尾 trim，`final_q6_decision_value=0`。因此 review 里的 “no-q6/control”
    需要按用途理解：风险控制口径不只是真无红，也包括对正式 decision 没有可规划红收益的局。
  - 2026-06-02 deep 高 trials 定向审计发现新的可观测分层：shape-less q6 品质点已经会进入
    `_quality_evidence_floors()` 的件数 floor，但因为没有 shape/cells，不会进入 layout footprint
    或 q6 格数 floor。`aisha_shipwreck_test_sample40_4rounds.json` 的 q6 local 为 `142`，
    结算红货为 `5 件 / 38 格 / 205.33万`，其中两个 `4x4`；`sample58` 的 q6 local 为 `137`，
    真实 q6 为 `318.57万`，但已知 layout bottom row 只有 `12`，因此原 deep gate 不会激活。
    全 333 个有效样本中 41 行包含 shape-less q6 品质点、合计 47 个点，没有任何真 0 红误命中；满足 Aisha
    shipwreck `shape+layout` 且 q6 品质点起始行 `>=13` 的只有上述 2 行。ratio 扫描显示
    `3.0` 可覆盖两行，但样本太少，不能直接上线。当前只新增 review-only
    `q6_quality_only_*` 字段和 flag，用 live 日志继续收证。
  - 2026-06-02 tail replacement truth 已结构化为第二审计轴。核心区别：
    `final_q6_decision_value` 仍然把未证据支持的极端 tail 裁掉，保持正式决策口径；
    `final_q6_decision_value_with_tail_replacement` 只在 review 中把被裁掉的 q6 tail 替换为同品质同形状普通红
    的估计值。live 路径会按当前 map 的 flattened drop pool 概率取 weighted P50；如果没有 map/drop
    权重，则退回 Item 表中位。`aisha_shipwreck_test_newsample7__normal_4rounds.json` 证明该字段能把
    `百年人参:103.9万` 记录为 `->15.894万`，同时保持 `shadow_affects_bid=0`。该结果说明之前“裁到 0”
    对正式保守出价是合理的，但做 shadow 升级复核时需要同时看 replacement truth，否则会低估可替代红货的小中额缺口。
  - 2026-06-02 抓包入口审计：`AuctionAnalyzer4.13.3` 反编译结果确认是 C# WPF + PaddleOCR
    桌面工具，抓取方式是 `Graphics.CopyFromScreen` 截图后 OCR，没有 WinDivert/Npcap/RawSocket
    等网络抓包实现，不能替代 Fatbeans 包流。Fatbeans 本地目录包含 `WinDivert64.sys`，但 WebHook
    受会员限制时不应破解；项目新增 `run_windivert_live_monitor.py` 作为自有合法只读抓包入口。
    初始检查 `pydivert=False`、`psutil=True`，随后已通过 `python -m pip install -e ".[packet]"`
    安装 `pydivert 3.1.3`，并确认 `Flag.SNIFF` 可用。网络适配器显示
    `Mihomo Meta Tunnel` Up、`WLAN` Up、`LetsTAP` 断开；在当前 Mihomo/TUN 状态下，
    `psutil.net_connections` 能识别 `BidKing.exe` 的 3 条 active flow，包括
    `198.18.0.1:* -> 8.133.195.27:10000` 和本地 loopback 对。这说明 VPN/TUN 下
    “按 BidKing.exe flow 归因”可行；但当前 PowerShell 非管理员，WinDivert payload sniff
    仍需用户用管理员 PowerShell 实测。脚本静态验证通过，单测用 fake packet/flow 覆盖了
    `SEND/REV` 映射与 Fatbeans row 转换。
  - 2026-06-02 实测 live raw 显示，使用道具后服务器会先发 `REV msg=0x0027` 直接响应，
    随后才在下一次 `REV push msg=0x0025` 状态同步中重复出现同一 action result。`0x0027`
    有效帧通常是 packet tag `0`、top-level field `2` 内嵌 action payload；空 ack 型
    `0x0027` 没有可用字段或 packet tag 非 0，应忽略。该 payload 不携带 session/map/round，
    只能继承当前已激活对局上下文，不能单独作为新局状态。
  - 2026-06-02 overlay 显示审计确认，`bid_rows` 已经有 `探价(P10)`、`防守价`、`抢仓上限`
    和 `停止价`，但旧 `ui_contract.baseline.decision` 只暴露 `attack_bid/stop_price`，
    且 mini 首屏优先显示“抢仓上限”。这会让用户把高风险上限误读为最可能价格；更合理的首屏
    是 `P50估值`、`防守价`、`当前最高` 和 q6 风险，完整 P10/P50/P90 与抢仓上限放入 hover/detail。
  - 2026-06-02 WinDivert 实机二次审计确认，权限问题修复后 monitor 确实在运行，但旧进程完整一局后
    状态仍为 `flows=1 / sniffed_packets=241 / raw_packets=1 / accepted_frames=0 /
    active_session_id=null`。`sniffed_packets` 证明 WinDivert 能看到系统 payload；
    `raw_packets≈0` 说明主要问题发生在 process-flow 归因层，而不是 auction frame gate。
    同时 `psutil.net_connections` 可见 `BidKing.exe` 既有
    `198.18.0.1:* -> 8.133.195.27:10000` 直连，也有本地 loopback 对和本地代理端口 `7897`。
    在 VPN / UU / TUN 场景中，真实游戏包可能经过 loopback 代理路径。提交 `d9de276` 已把
    loopback flow 默认纳入 WinDivert process-match；下一步必须重启后实战验证，不能把代码落地
    直接当成抓包闭环完成。
  - 2026-06-02 overlay 闪烁回归的根因不是 Tk hover/detail 本身，而是刷新签名包含
    `capture_source_status.json` 的文件 mtime。WinDivert 状态文件约每两秒重写一次，即使只有
    timestamp 或 `sniffed_packets` 心跳变化，也会触发整窗 render。提交 `d9de276` 改为语义签名：
    忽略 timestamp-only / sniffed-only 变化，只在 source、flow、raw、accepted、session 等
    用户可感知状态变化时重绘。
...
## 2026-06-04 live 19:03/19:06/19:10 三局观察

- 19:03 Ethan `2502` R2 在 `100104` 普品扫描后，formal posterior 明显坍缩：pre-bid replay 中 truth
  `461,226`，R2 P50 约 `89,443`、P90 约 `337,087`；R3 继续偏低。诊断标签集中在
  `q6_below_drop_prior`、`q6_count/cells_under` 和 `q6_tail_value`，不是 UI 显示错误。
- 同一局结算后还收到一条 `0x0027` direct action，内容属于下一局 Ethan `2501` 的首道具。该现象确认
  `0x0027` 本身不带 session/map，只能依赖 gate 上下文；跨局时必须暂存到下一 session，不能继承上一局。
- 19:06 Ethan `2501` 首条可用完整 state 晚于第一条 bid，旧 raw 因此 R1 `no_state`。按新 gate
  拼接回放后，上一局结算后的 direct action 可作为该局 SortID=1，能改善新局开头衔接与 R1 pre-bid 可用性。
- 19:10 Aisha `2405` R2/R3/R4 表现相对正常：formal P50 低估但 P90 覆盖，属于当前可接受区间。
- 本轮新增的三局样本中，Aisha 2405 是 normal calibration 行；Ethan 2502 是 q6 false-low/tail event
  代表行；Ethan 2501 暴露 exact bucket 约束过紧，需要后续单独看 q1/q3 exact cells 和 q4 exact value。

## 2026-06-04 live 19:30/19:33/19:36 三局观察

- `19:30` Ethan `2501` 是当前最强的实战低估样本：正式结算 `1,338,112`，q6 `3 件 / 25 格 /
  854,210`，但 formal q6 P90 为 `0`，整体 formal P90 只有约 `187,813`。布局 bottom row `19`、
  evidence profile `layout`，符合 Ethan shipwreck conditional sampler 的门控；10 次 shadow 可把 q6
  风险参考抬到约 `996,850`，但 formal 停止价不变。
- `19:33` Aisha `2401` 是较正常的 Villa 样本：P90 覆盖，P50 偏保守；`aisha_villa_floor05`
  shadow 可提高 q6 覆盖，但因历史 active no-q6/control 误抬，仍不合并为实战参考。
- `19:36` Aisha `2410` 是 high-info q6 compact miss：结算 `352,169`，q6 `122,600`，formal q6
  P90 为 `0`，整体 P90 低约 `90k`。它说明 Aisha Villa 仍有优化空间，但当前证据不足以用统一 floor
  放宽。
- 第三局切换问题不是上一局污染：`2401` 结算后到 `2410` 第一条 `0x0022` 报价前，raw 中没有任何带
  新 session 的游戏帧；只有无 session 心跳类 ignored frames。因此当前网络路径无法在该类局首报价前推断新
  session/map。若新局先发 `0x0026` 道具发送，已补 gate 可用该帧切换。

## 2026-06-04 live 19:55-20:05 四局公开信息与跨局观察

- 第一局 Aisha `2403` 的公开总格数 `200009=98` 从 `0x0021` 起已存在，数值在 protobuf field `14`；
  旧 parser 只尝试 field `11/9`，所以该公开精确信息没有进入模型，UI 只能显示 layout 估格。
- 同类历史扫描确认 exact numeric 字段分两组：`200009-200012` 使用 field `14`，
  `200017-200020` 使用 field `7`。接入后，当前观察到的 public info id 已无 unknown/pending。
- 第四局 Aisha `2403` 的公开总件数为 `200017=38`，而结算 inventory parser 只得到 `36` 个唯一物品；
  raw 中有 `37` 个 item-like node，且有一个重复 runtime/item pair。公开 exact 本身不应因此降级，
  该差异应标记为结算解析/采集数据质量问题。
- 前两局开局有 `0x0021`，monitor 可立即识别；后两局没有 `0x0021`，最早可识别的新局帧是首个 action
  或 bid/state。当前没有证据说明 VPN 是直接根因；VPN/TUN 只可能通过包丢失或 flow 归因间接放大该现象。
- exact 总格数接入后，历史 cells-only 样本暴露 residual sampler rejection 缺口。新增 exact-cells fill
  后，`aisha_hidden_test_sample4_4rounds.json`、`ethan_hidden_test_sample3_4rounds.json`、
  `ethan_villa_test_sample29_2rounds.json` 从 zero/低匹配恢复为可采样；剩余 zero-match 主要是多桶
  exact cells/value/shape 组合冲突，不能继续归因于公开总格数。

## 2026-06-04 live 20:35-20:49 七局观察

- `20:37:53` Aisha `2401` 与 `20:48:01` Ethan `2407` 是这轮两局开局未立即显示的代表：
  二者 raw 的第一帧都是 `SEND 0x0026`，均带新 session；但没有 `0x0021`，第一条可解析 state
  分别到 sort `9` / `8` 的 `0x0025`，且都晚于首个 `SEND 0x0022` 报价。
- 因此当前现象不再是“完全没有抓到新局”，而是“抓到 session/action，但缺首个 state”。VPN/TUN 仍可能是
  造成首状态包不可见或服务端不推 `0x0021` 的外部条件，但本地可见证据只能证明：有些局的 `0x0021`
  确实不在 raw 中。
- 这类局能做两层处理：UI 层立即隐藏旧局并显示“新局监听中”；模型层不能在没有 state/hero/bids 的情况下
  给正式估值。若后续要继续提前估价，需要研究是否能从 `SEND 0x0026` 的 session/action、status 玩家序列
  或上一局上下文中安全恢复 hero/round；当前先不伪造。
- 新 7 局全部有 q6，估值低估集中在 Ethan Villa 和 Aisha Villa q6 value/count/cells 分布：
  `zero_match=0`、`layout_conflict=0`，说明问题不是解析失败，而是 posterior 分布仍保守。
