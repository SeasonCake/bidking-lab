...
2. `adaptive_filter` fallback 重建 bucket 时丢失 `huge_cells_override`（仅保留 `huge_band`）。
...
- **下一项 C-32**：仅 **P0-B** — `adaptive_filter` fallback 重建时保留 `huge_cells_override`（`posterior.py` 256–272 行一带）。

### 后续可选优化

见 PROGRESS.md「下一步推进 TODO」与 backlog 归档 — P2/P3 仍暂缓。
